...
router.get('/list/:user_id',  isAuth, async (req:IRequest, res:Response) => {
    ...
    try{
        const friend_list_1 = await FriendshipRepository.findBy({user1: user})
        const friend_list_2 = await FriendshipRepository.findBy({user2: user})
...
router.post('/create/',  isAuth, async (req:IRequest, res:Response) => {
    if(('user_id_1' in req.body && 'user_id_2' in req.body)){
        const user_1 = await UserRepository.findOneBy({id: req.body.user_id_1})
        const user_2 = await UserRepository.findOneBy({id: req.body.user_id_2})
        if(!user_1 || !user_2)
            return res.status(400).send('Error server not found')
...
router.delete('/delete/:friendship_id', isAuth, async (req:IRequest, res:Response) => {
...
router.get('/sent_request_list/:user_id', isAuth, async (req:IRequest, res:Response) => {
    ...
    try{
        const friend_request_list = await FriendRequestRepository.findBy({sender: user})
...
router.post('/send_request/', async (req:IRequest, res:Response) => {
    if(('sender_id' in req.body && 'receiver_id' in req.body)){
        const sender = await UserRepository.findOneBy({id: req.body.sender_id})
        const receiver = await UserRepository.findOneBy({id: req.body.receiver_id})
        if(!sender || !receiver)
            return res.status(400).send('Error server not found')
...
router.delete('/delete_request/:request_id', isAuth, async (req:IRequest, res:Response) => {
